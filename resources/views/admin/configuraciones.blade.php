...
<style>
    ...
    .btn-primary-custom {
        background-color: #0f172a; color: #ffffff; border: none; padding: 0.6rem 1.25rem;
        border-radius: 8px; font-weight: 500; font-size: 0.9rem; display: flex; align-items: center;
        gap: 0.5rem; cursor: pointer; transition: all 0.2s ease; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);
    }
    .btn-primary-custom:hover { background-color: #1e293b; transform: translateY(-1px); }

    .btn-secondary {
        background-color: #ffffff; color: #334155; border: 1px solid #e2e8f0; padding: 0.5rem 1rem;
        border-radius: 8px; font-weight: 500; font-size: 0.9rem; display: flex; align-items: center;
    ...
    .tab-btn {
        background-color: transparent; color: #64748b; border: none; padding: 0.6rem 1.25rem;
        border-radius: 6px; font-weight: 600; font-size: 0.9rem; cursor: pointer; transition: all 0.2s ease;
    ...
    .search-box input {
        width: 100%; padding: 0.55rem 1rem 0.55rem 2.8rem; border: 1px solid #e2e8f0;
        border-radius: 8px; font-size: 0.9rem; outline: none; transition: 0.2s; box-sizing: border-box;
    }
    .search-box input:focus { border-color: #3b82f6; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1); }

    .custom-table { width: 100%; border-collapse: collapse; }
    .custom-table th {
        background-color: #f8fafc; color: #1e293b; font-size: 0.75rem; text-transform: uppercase;
        letter-spacing: 0.05em; font-weight: 800; padding: 1.2rem 1.5rem; text-align: left; border-bottom: 2px solid #e2e8f0;
    }
    ...
    .btn-icon {
        background: transparent; border: none; color: #94a3b8; width: 36px; height: 36px; border-radius: 8px;
        display: flex; align-items: center; justify-content: center; cursor: pointer; transition: 0.2s; font-size: 1.25rem;
    }
    .btn-icon:hover { background-color: #f1f5f9; color: #0f172a; }
    .btn-icon.delete:hover { background-color: #fef2f2; color: #ef4444; }

    .table-footer {
        padding: 1rem 1.5rem; background-color: #ffffff; display: flex; justify-content: space-between;
...
<div class="dashboard-premium">
    ...
    <div class="table-card">
        <div class="toolbar">
            ...
            <div id="info-registros" style="color: #64748b; font-size: 0.85rem; font-weight: 600;">
...
<div id="modal-formulario" class="modal-overlay">
    <div class="modal-content">
        ...
        <form id="formulario-datos" onsubmit="guardarRegistro(event)">
            ...
            <div class="form-group" id="grupo-descripcion">
                <label>Descripción / Observaciones:</label>
                <textarea id="registro-descripcion" rows="3"></textarea>
            </div>
            ...
            <div class="modal-acciones">
                ...
                <button type="submit" class="btn-primary-custom" style="margin: 0; box-shadow: none; width: 100%; justify-content: center;">Guardar Datos</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function filtrarDatos() {
        ...
        datosFiltrados = datosCompletos.filter(item => {
            ...
            return nombre.includes(termino) || desc.includes(termino) || id.includes(termino);
        });
        ...
        datosPagina.forEach(item => {
            ...
            if(!esModuloEstado) {
                fila += `
                    ...
                    <td>
                        <span class="badge ${infoEstado.clase}">
                            <i class='bx ${infoEstado.icono}'></i> ${infoEstado.texto}
                        </span>
                    </td>`;
            }
    ...
    function abrirModalEliminar(id, nombre, descripcion) {
        ...
        if (moduloActual === 'estados') {
            document.getElementById('eliminar-desc-container').style.display = 'none';
        } else {
            document.getElementById('eliminar-desc-container').style.display = 'block';
            document.getElementById('eliminar-desc-text').innerText = descripcion || 'Sin descripción';
        }
    ...
    async function guardarRegistro(event) {
        ...
        if (moduloActual !== 'estados') {
            payload.descripcion = document.getElementById('registro-descripcion').value;
            payload.estado_id = document.getElementById('registro-estado').value;
        }

        const endpoint = id ? `${urlBase}/${moduloActual}/actualizar` : `${urlBase}/${moduloActual}/crear`;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        cambiarModulo('categorias', 'Categoría', 'Nueva Categoría');
    });
</script>
@endsection