...
<style>
    ...
    .btn-secondary {
        background-color: #ffffff; color: #334155; border: 1px solid #e2e8f0; padding: 0.6rem 1rem;
        border-radius: 8px; font-weight: 500; font-size: 0.9rem; display: flex; align-items: center; gap: 0.4rem; cursor: pointer;
    }
    ...
    <div class="table-card">
        <div class="toolbar">
            <div class="search-box">
                ...
                <input type="text" id="inputBuscarUsuario" placeholder="Buscar por nombre o correo..." oninput="filtrarDatos()">
            </div>
    ...
    <div class="modal-content">
        ...
        <form id="formulario-datos" onsubmit="guardarRegistro(event)">
            ...
            <div class="form-group">
                ...
                <input type="text" id="usuario-nombre" required placeholder="Ej: Dr. Carlos Mendoza">
            </div>
            
            <div class="form-group">
                <label>Correo Electrónico:</label>
                <input type="email" id="usuario-email" required placeholder="Ej: cmendoza@hospital.com">
            </div>
...
<script>
    ...
    async function inicializar() {
        try {
            const [usuariosRes, rolesRes] = await Promise.all([
                ...
                fetch(`${urlBase}/roles/obtener/todos`)
            ]);
    ...
    function poblarRoles() {
        ...
        rolesList.forEach(r => { 
            selectRol.innerHTML += `<option value="${r.id}">${r.nombre || r.name}</option>`; 
        });
    }
    ...
    function renderizarTabla() {
        ...
        datosPagina.forEach(u => {
            ...
            // EVALUACIÓN VISUAL DE ACCIONES (Bloqueo de Perfil Propio)
            let accionesHtml = '';
            if (u.id === currentUserId) {
                // Si es el usuario actual, mostramos una etiqueta indicando que es su cuenta
                accionesHtml = `<span style="background-color: #f1f5f9; color: #64748b; padding: 4px 10px; border-radius: 6px; font-size: 0.75rem; font-weight: 700; border: 1px solid #e2e8f0;">Tu Cuenta</span>`;
            } else {
                // Si es otro usuario, mostramos los botones de acción normales
                accionesHtml = `
    ...
    function abrirModalEliminar(id, nom, email) { 
        idAEliminar = id; 
        document.getElementById('eliminar-nombre-text').innerText = nom; 
        document.getElementById('eliminar-email-text').innerText = email; 
    ...
    async function guardarRegistro(e) {
        ...
        const payload = { 
            ...
            role_id: document.getElementById('usuario-rol').value, 
            estado_id: document.getElementById('usuario-estado').value 
        };
        ...
        const endpoint = id ? `${urlBase}/usuarios/actualizar` : `${urlBase}/usuarios/crear`;
        ...
        try {
            ...
            if (res.ok) { 
                cerrarModal(); 
                mostrarToast(id ? 'El usuario ha sido actualizado correctamente.' : 'El usuario fue registrado con éxito.', 'success');
                const r = await fetch(`${urlBase}/usuarios`); 
                usuariosCompletos = await r.json(); 
                filtrarDatos(); 
            } else { 
                mostrarToast(result.message || 'Verifica los datos ingresados.', 'error'); 
            }
        } catch(err) { 
            console.error(err); 
    ...
    async function confirmarEliminacion() {
        ...
        try {
            ...
            if (res.ok) { 
                cerrarModalEliminar(); 
                mostrarToast('La cuenta de usuario ha sido eliminada.', 'success');
                const r = await fetch(`${urlBase}/usuarios`); 
                usuariosCompletos = await r.json(); 
                filtrarDatos(); 
            } else { 
                mostrarToast(result.message || 'No se pudo eliminar la cuenta.', 'error'); 
                cerrarModalEliminar(); 
            }
        } catch(err) { 
            console.error(err); 
