...
<style>
    ...
    .btn-primary-custom { background-color: #0f172a; color: #ffffff; border: none; padding: 0.6rem 1.25rem; border-radius: 8px; font-weight: 500; font-size: 0.9rem; display: flex; align-items: center; gap: 0.5rem; cursor: pointer; }
    ...
    .avatar { width: 42px; height: 42px; border-radius: 10px; background: #eff6ff; color: #2563eb; display: flex; align-items: center; justify-content: center; font-weight: 800; border: 1.5px solid #dbeafe; flex-shrink: 0; }

    .status-badge { display: inline-flex; align-items: center; gap: 0.35rem; padding: 0.4rem 0.9rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 800; background-color: #ecfdf5 !important; color: #047857 !important; border: 1px solid #10b981 !important; }
    ...
    <div class="table-card">
        <div class="toolbar">
            <div class="search-box">
                ...
                <input type="text" id="inputBuscarInsumo" placeholder="Buscar por nombre o código..." oninput="filtrarDatos()">
            </div>
    ...
    <div class="modal-content">
        ...
        <form id="formulario-datos" onsubmit="guardarRegistro(event)">
            ...
            <div class="form-group">
                ...
                <input type="text" id="insumo-nombre" required placeholder="Ej: Ibuprofeno 400mg">
            </div>
            
            <div class="form-row">
                <div class="form-group">
                    <label>Código de Referencia:</label>
                    <input type="text" id="insumo-codigo" placeholder="Ej: MED-IBU-001">
                </div>
                <div class="form-group">
                    <label>Stock Mínimo:</label>
                    <input type="number" id="insumo-stock" placeholder="Ej: 50" min="0">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="insumo-proveedor" required>
            ...
            <div class="form-group">
                ...
                <textarea id="insumo-descripcion" rows="2" placeholder="Caja con 20 tabletas..."></textarea>
            </div>

            <div class="form-group" id="grupo-estado">
                <label>Estado del Insumo:</label>
                <select id="insumo-estado">
            ...
            <div class="modal-acciones">
                ...
                <button type="submit" class="btn-primary-custom" style="margin: 0; box-shadow: none; width: 100%; justify-content: center;">Guardar Insumo</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    // Carga paralela: Insumos, Proveedores y Categorías
    async function inicializar() {
        try {
            const [insumosRes, provRes, catRes] = await Promise.all([
                fetch(`${urlBase}/insumos`),
                fetch(`${urlBase}/proveedores/obtener/todos`),
                fetch(`${urlBase}/categorias/obtener/todos`)
            ]);
            
            insumosCompletos = await insumosRes.json();
    ...
    // Llenamos los <select> del modal automáticamente
    function poblarSelects() {
        const selectProv = document.getElementById('insumo-proveedor');
        const selectCat = document.getElementById('insumo-categoria');
    ...
    function renderizarTabla() {
        ...
        const datosPagina = insumosFiltrados.slice(inicio, fin);

        datosPagina.forEach(i => {
            const iniciales = i.nombre ? i.nombre.substring(0, 2).toUpperCase() : 'IN';
            
            // Buscamos los nombres reales cruzando datos
            const prov = proveedoresList.find(p => p.id == i.proveedor_id);
            const nombreProveedor = prov ? prov.nombre : 'Sin proveedor';
            
            const cat = categoriasList.find(c => c.id == i.categoria_id);
            const nombreCategoria = cat ? cat.nombre : 'Sin categoría';

            const estadoId = i.estado_id || 1;
            const state = estadosMap[estadoId] || estadosMap[1];
                ...
                <tr>
                    ...
                    <td>
                        <div class="item-details">
                            ...
                            <span>Insumo ID: #${i.id}</span>
                        </div>
                    </td>
                    <td>
                        <div class="item-details">
                            <strong>${nombreProveedor}</strong>
                            <span>Categoría: ${nombreCategoria}</span>
                        </div>
                    </td>
                    <td>
                        <div class="item-details">
                            <strong>Mínimo: ${i.stock_minimo || 0}</strong>
                            <span class="status-badge" style="background-color: ${state.bg} !important; color: ${state.text} !important; border-color: ${state.border} !important; margin-top: 4px;">
                                <i class='bx ${state.icono}' style="color: ${state.text} !important;"></i> ${state.texto}
                            </span>
                        </div>
                    </td>
                    <td>
                        <div style="display: flex; justify-content: flex-end; gap: 5px;">
                            ...
                            <button class="btn-icon" onclick="abrirModalEliminar(${i.id}, '${i.nombre}', '${i.codigo || 'N/A'}')" style="transition: 0.2s;" onmouseover="this.style.color='#ef4444'; this.style.backgroundColor='#fee2e2';" onmouseout="this.style.color='#94a3b8'; this.style.backgroundColor='transparent';"><i class='bx bx-trash'></i></button>
                        </div>
                    </td>
                </tr>
            `;
    ...
    function abrirModalCrear() {
        ...
        // ocultamos el estado al crear, ya que por defecto será "Activo" y no es necesario mostrarlo
    ...
    function abrirModalEditar(itemString) {
        ...
        document.getElementById('insumo-stock').value = i.stock_minimo || 0;
        document.getElementById('insumo-proveedor').value = i.proveedor_id || '';
        ...
        document.getElementById('insumo-estado').value = i.estado_id || 1;
        
        // mostramos el estado al editar, ya que es relevante para la actualización
    ...
    // Funciones API (Guardar y Eliminar)
    async function guardarRegistro(event) {
        ...
        const payload = {
            ...
            stock_minimo: parseInt(document.getElementById('insumo-stock').value) || 0,
            proveedor_id: document.getElementById('insumo-proveedor').value,
            categoria_id: document.getElementById('insumo-categoria').value,
            descripcion: document.getElementById('insumo-descripcion').value,
            estado_id: document.getElementById('insumo-estado').value
        };

        const endpoint = id ? `${urlBase}/insumos/actualizar` : `${urlBase}/insumos/crear`;
        ...
        try {
            ...
            if (response.ok) {
                cerrarModal();
                mostrarToast(id ? 'El insumo ha sido actualizado correctamente.' : 'El insumo fue registrado con éxito.', 'success');
                const res = await fetch(`${urlBase}/insumos`); 
                insumosCompletos = await res.json();
                filtrarDatos();
            } else {
                mostrarToast('Error al guardar. Verifique los datos o que el proveedor exista.', 'error');
            }
        } catch (error) {
    ...
    async function confirmarEliminacion() {
        ...
        try {
            const response = await fetch(`${urlBase}/insumos/eliminar/${idAEliminar}`, { method: 'DELETE' });
            if (response.ok) {
                cerrarModalEliminar();
                mostrarToast('El insumo ha sido eliminado de la base de datos.', 'success');
                const res = await fetch(`${urlBase}/insumos`); 
                insumosCompletos = await res.json();
                filtrarDatos();
            } else {
                mostrarToast('No se pudo eliminar el insumo. Verifique que no esté en uso.', 'error');
                cerrarModalEliminar();
            }
        } catch (error) {
            console.error('Error:', error);
