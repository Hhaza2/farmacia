<?php

namespace App\Policies;

use App\Models\Lote;
use App\Models\User;
use Illuminate\Auth\Access\Response;

class LotePolicy
{
    /**
     * Determine whether the user can view any models.
     */

public function registrarEntrada(User $user, Lote $lote): bool
    {
        return in_array($user->role_id, [1, 2]);
    }

    // Admin (1), Farmacéutico (2) y Enfermería (3)
    public function registrarSalida(User $user, Lote $lote): bool
    {
        return in_array($user->role_id, [1, 2, 3]);
    }
    public function viewAny(User $user): bool
    {
        return false;
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, Lote $lote): bool
    {
        return false;
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return false;
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, Lote $lote): bool
    {
        return false;
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, Lote $lote): bool
    {
        return false;
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, Lote $lote): bool
    {
        return false;
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, Lote $lote): bool
    {
        return false;
    }
}
